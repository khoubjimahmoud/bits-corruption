...
def plot_intervals_before_peaks(bit_corruption_freq, grouped_peaks, interval_size=100):
    """
    Tracer un histogramme des fréquences de corruption pour les intervalles juste avant chaque pic détecté.

    :param bit_corruption_freq: Tableau contenant la fréquence de corruption des bits.
    :param grouped_peaks: Liste des positions des pics détectés.
    :param interval_size: Taille de l'intervalle avant chaque pic.
    """
    ...
    ax.set_title("Fréquence de corruption avant les pics détectés")
    ax.legend()
    plt.show()
...
# Tracer les intervalles avant les pics
if bit_corruption_freq is not None:
    plot_intervals_before_peaks(bit_corruption_freq, grouped_peaks, interval_size=100)
else:
